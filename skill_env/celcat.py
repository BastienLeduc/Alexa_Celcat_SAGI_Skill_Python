...
def parse_cmd(message):
    ...
    return datetime.datetime.strptime(message, "%Y-%m-%d")
...
def get_icalendar():
    if os.path.exists(ics_file):
        ...
        created_time = datetime.datetime.strptime("2019-04-10", "%Y-%m-%d")
    ...
    if created_time + datetime.timedelta(hours=1) < datetime.datetime.now():
            ...
            open("/tmp/"+"ics_etu.ics", 'wb').write(r.content)    

    return Calendar.from_ical(open("/tmp/"+"ics_etu.ics", 'rb').read())
...
def format_courses(courses):
    ...
    if courses:
        ...
        for course in courses:
            f = "{:%Hh%M} à {:%Hh%M} : {} {} en {} avec {} ({})".format(course["debut"], course["fin"],
